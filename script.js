...
monthNumber.addEventListener('input',()=>{
  ...
  if(monthDigit.length == 1)
    ...
    expiryDateEl.innerHTML = monthNumber.value + "/" + yearNumber.value;
...
yearNumber.addEventListener("input",()=>{
  ...
  if (expiryDateEl.innerHTML != "" && yearDigit.length > 0)
    ...
    expiryDateEl.innerHTML = monthNumber.value + "/";
...
form.addEventListener('submit',(e)=>{
  ...
  if (!regexMonth.test(monthNumber.value))
      blanks[0].style.display = "block";
  else
    blanks[0].style.display = "none";
  if (!regexYear.test(yearNumber.value))
    blanks[0].style.display = "block";
  else
    blanks[0].style.display = "none";
  if (!regexCvc.test(cvc.value))
    blanks[1].style.display = "block";
  else
  blanks[1].style.display = "none";
  if (!regexDigit.test(cardNumber.value) || !regexMonth.test(monthNumber.value) 
  || !regexName.test(fullName.value) || !regexYear.test(yearNumber.value) || !regexCvc.test(cvc.value)) return  
